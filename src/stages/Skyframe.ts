...
export function buildSkyframe(ctx: StageContext): THREE.Vector3 {
  ...
  // y[0,2.2] は1F出入口（地上からシャフトへ入る＝壁ジャン直登の起点）として開ける
  span(-0.75, 0.75, 2.2, F2, 2.0, SH, C_CONCRETE2); // 1F扉の上〜2F床
  ...
  // ====================================================================
  // 東面を上る階段状の足場（各段+1.2m・水平約3mの斜め上ジャンプで届く＝低速だが安全）。
  // 段を外しても1.2m下の段に受け止められる。最後は屋上の堅い床(12,-12)へ渡る。
  // [中心x, 上面高さ, 中心z]
  const scaffold: Array<[number, number, number]> = [
    [16, 1.2, 12],
    [16, 2.4, 9],
    [16, 3.6, 6],
    [16, 4.8, 3],
  ...
  for (const [bx, by, bz] of scaffold) {
    slab(bx - 1.4, bx + 1.4, bz - 1.4, bz + 1.4, by, C_BOARD);
    vcyl(0.05, 0.05, by, bx + 1.2, by / 2, bz + 1.2, C_STEEL_LT); // 支柱（視覚）
    vcyl(0.05, 0.05, by, bx - 1.2, by / 2, bz - 1.2, C_STEEL_LT);
  }
  // 入口板（ヤード側）。板を1枚欠落させ約3.2mの隙間ジャンプで最初の段(16,12)へ。
  // 外しても1.2m下のヤードに落ちるだけの低リスクな緊張。
  slab(14.6, 17.4, 16.6, 19.4, 1.2, C_BOARD);

  // ====================================================================
  // 6. 資材スタックの屋内ルート（地上→2F→3F→屋上）
  // ====================================================================
  // 各段は載っている床から+1.2m・水平約3m以内（ジャンプ高1.42mで確実に届く）。
  const stack = (
    x: number,
    z: number,
    restY: number,
    ...
    addBox(w, topY - restY, w, x, (restY + topY) / 2, z, color);
  };
  // 地上→2F：南ヤードからタワーへ寄せる木箱段。上面1.2/2.4/3.6 → 2F南縁(z=15)へ短いホップ
  stack(0, 24, 0, 1.2, 2.4, C_FORMWORK);
  stack(0, 21, 0, 2.4, 2.4, 0x8f7a3a);
  stack(0, 18, 0, 3.6, 2.6, C_FORMWORK);
  // 2F→3F：2Fの床(3.6)に載せた型枠段。上面4.8/6.0/7.2 → 3F(z=3付近)へ
  stack(8, 9, F2, 4.8, 2.2, 0x8f7a3a);
  stack(8, 6, F2, 6.0, 2.2, C_FORMWORK);
  stack(8, 3, F2, 7.2, 2.4, 0x8f7a3a);
  // 3F→屋上：3Fの床(7.2)に載せたサポート資材段。上面8.4/9.6/10.8 → 屋上(z=-2)へ
  stack(8, -8, F3, 8.4, 2.2, C_FORMWORK);
  stack(8, -5, F3, 9.6, 2.2, 0x8f7a3a);
  stack(8, -2, F3, 10.8, 2.4, C_FORMWORK);
